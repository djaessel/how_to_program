...
def command_help():
    print_help_info("CALL", "Ruft eine Batchdatei von einer anderen Batchdatei aus auf.")
    ...
    print_help_info("TOUCH", "Erstellt eine leere Datei.")
...
def command_touch(command_data):
    l = len(command_data)
    if l == 2:
        path = command_data[1]
        if not os.path.exists(path):
            ...
            print("Die angegebene Datei existiert bereits!")
    else:
        print("Syntaxfehler.")


def command_call(command_data):
    l = len(command_data)
    if l >= 2:
        path = command_data[1]
        if os.path.exists(path) and os.path.isfile(path):
            ...
            print(f'Der Befehl "{path}" ist entweder falsch geschrieben oder')
